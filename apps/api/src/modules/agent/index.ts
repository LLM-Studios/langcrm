...
export class Agent {
     ...
     * ```
     */
    public async invoke(params: {messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[], prompt_extra?: string}): Promise<string | null> {
        ...
        if (message.tool_calls) {
            return this.handleToolCalls({messages: [...messages, message], toolCalls: message.tool_calls});
        }

        return message.content;
    }

    private async handleToolCalls(params: {messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[], toolCalls: OpenAI.Chat.Completions.ChatCompletionMessageToolCall[]}) {
