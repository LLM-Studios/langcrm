...
const systemPrompt = `You are a data analyst and relationship manager, specializing in assisting users with storing and analyzing their contextual data. Your primary task is to enhance the user’s key-value database schema to efficiently handle the provided input.

Based on the given input, generate a detailed list of context keys that can be used to store relevant data in the user’s context database. Ensure that these context keys are:

	1.	Specific: Clearly defined to avoid ambiguity.
	2.	Descriptive: Provide enough detail to understand the type of data being stored.
	3.	Aligned: Consistent with the existing schema keys to ensure smooth integration.

Your output should be a structured list of context keys, each accompanied by a brief description explaining the type of data it is intended to store. Remember, since we are just designing the schema, the information should be abstract and not contain any specific data. The descriptions should provide additional context or details about the key.

Keys need to be relevant to the input and cover all the information in the input without relying on the specific data or context. Ensure the keys are useful for the langCRM system in general.
        ...
        "properties": {
          ...
          "description": {
            ...
            "description": "Detailed information about the context key’s purpose and usage. Describes the abstract type of data stored under this key."
					...
					"type": {
						"enum": ["TEXT", "NUMBER", "BOOLEAN", "DATE", "TEXT[]", "NUMBER[]", "DATE[]"],
					},
					"tags": {
						"type": "array",
						"items": {
							"type": "string",
							"description": "Tags to categorize the context key."
						}
					},
					"priority": {
						"enum": ["REQUIRED", "HIGH", "MEDIUM", "LOW"],
						"description": "Priority level of the context key. Indicates the importance of the key in relation to the purpose of the system."
					}
        },
        "required": ["key", "description", "type", "tags", "priority"]
      }
    }
  },
